...
import { deletePurchase, getPurchasesWatting } from '../Cart/cart.slice'
...
export default function Purchase() {
  ...
  const handleActiveClass =
    location.pathname === path.purchase ? 'active-purchase' : ''
  const handleCancelPurchase = id => async () => {
    await dispatch(deletePurchase([id])).then(unwrapResult)
    await dispatch(getPurchasesWatting())
      .then(unwrapResult)
      .then(res => {
  ...
  return (
    <div className="container">
      <S.PurchaseWrapper>
        <S.PurchaseProfile>
          <S.PurchaseProfileUser>
            <S.PurchaseProfileUserAvatar>
              <img
          ...
          <S.PurchaseProfileBar>
            <S.PurchaseProfileBarItem to="">
              <img
                src="https://cf.shopee.vn/file/12bc9caf9344342250a67bc34cde32c3"
                alt=""
              />
              tết sale
            </S.PurchaseProfileBarItem>
            <S.PurchaseProfileBarItem to={path.user}>
              <img
        ...
        <S.PurchaseContent>
          <S.PurchaseContentHeader>
            <S.PurchaseContentHeaderAll>Tất cả</S.PurchaseContentHeaderAll>
            <S.PurchaseContentHeaderWatting>
              Chờ xác nhận
            </S.PurchaseContentHeaderWatting>
            <S.PurchaseContentHeaderOnGetting>
              Chờ lấy hàng
            </S.PurchaseContentHeaderOnGetting>
            <S.PurchaseContentHeaderOnDelivery>
              Đang giao
            </S.PurchaseContentHeaderOnDelivery>
            <S.PurchaseContentHeaderReceived>
              Đã giao
            </S.PurchaseContentHeaderReceived>
            <S.PurchaseContentHeaderCancel>
              Đã hủy
            </S.PurchaseContentHeaderCancel>
          </S.PurchaseContentHeader>
          <S.PurchaseContentSection>
            {purchases &&
              purchases.map(purchase => (
                <S.PurchaseContentSectionItem key={purchase._id}>
                  <S.PurchaseContentSectionItemInfor>
                    <S.PurchaseContentSectionItemInforLeft>
                      <S.PurchaseContentSectionItemInforLeftImage>
                        <img src={purchase.product.image} alt="" />
                      </S.PurchaseContentSectionItemInforLeftImage>
                      <S.PurchaseContentSectionItemInforLeftName>
                        <p>{purchase.product.name}</p>
                        <span className="category">
                          Phân loại hàng: {purchase.product.category.name}
                        </span>
                        <br />
                        <span>x{purchase.buy_count}</span>
                      </S.PurchaseContentSectionItemInforLeftName>
                    </S.PurchaseContentSectionItemInforLeft>
                    <S.PurchaseContentSectionItemInforRight>
                      <p className="origin">
                  ...
                  <S.PurchaseContentSectionItemAction>
                    <div></div>
                    <S.PurchaseContentSectionItemActionPrice>
                      Tổng số tiền:
                      <span>
                        đ
                        {formatMoney(
                    ...
                    <S.PurchaseContentSectionItemActionCancel
                      onClick={handleCancelPurchase(purchase._id)}
                    >
                      hủy đơn hàng
                    </S.PurchaseContentSectionItemActionCancel>
                  </S.PurchaseContentSectionItemAction>
                </S.PurchaseContentSectionItem>
              ))}
          </S.PurchaseContentSection>
        </S.PurchaseContent>
      </S.PurchaseWrapper>
